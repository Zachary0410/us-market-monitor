...
它会依次做四件事，最后把日报写到 `reports\YYYY-MM-DD.md` 并打印到终端：

```
[1/4] 取数 ...      取回三个指数的日线；取不到的用 data\ 里的缓存兜底
[2/4] 算指标 ...    涨跌幅、均线、相对均线偏离
[3/4] 判断异常 ...  按 config.py 里的阈值判 正常 / 关注 / 警报
[4/4] 生成日报 ...  写入 reports\，并打印
```
...
> 注意：报告文件名用的是香港时间的日期。香港时间 9 月 15 日早上 6 点生成的报告，文件名是 `2026-09-15.md`，但里面写的"数据日期"是 9 月 14 日（美东收盘那天）。**看数据永远以"数据日期"为准。**
...
第一阶段已经完成——四个模块各管一层，加一个入口把它们串起来：

1. ✅ `fetch.py` —— 取回三个指数的真实日线并缓存到 `data\`
2. ✅ `metrics.py` —— 算出涨跌幅、均线、相对均线偏离
3. ✅ `signals.py` —— 按标的阈值判异常并写出理由
4. ✅ `report.py` —— 生成 Markdown 日报
5. ✅ `run_daily.py` —— 一键跑完整条流水线

接下来可以做的事（大致按价值排序）：

1. **定时任务** —— 让 Windows 任务计划程序在每个交易日收盘后自动跑一次
2. **阈值校准** —— 用历史数据统计"过去十年这条规则触发过几天"，把拍脑袋定的阈值换成有依据的
3. **历史对比** —— 每天把关键数字追加到一份 CSV，就能看趋势而不只是看今天
4. **Streamlit 界面** —— 把日报做成网页，鼠标点一点就能看
