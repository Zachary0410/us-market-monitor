...
- **缓存**：用 `@st.cache_data(ttl="30m")` 缓存读数，避免每个访客、每次点击都重新取数；点「立即更新」会清掉缓存，保证看到的是最新的。
...
| 位置 | 内容 |
| --- | --- |
| 顶部 | 三个标的的卡片：最新收盘、单日涨跌、当天结论 |
| 走势 | 可切"最近 30 / 90 / 250 个交易日"，一个标的一个标签页，另加一个"标普 vs 纳指"对比 |
| 历史日报 | 选一天，直接看那天生成的日报正文和走势图 |
| 页尾 | 历史表的最后 20 行原始数据 |
| 左边栏 | 「立即更新」按钮 + 上次运行的完整输出 |
